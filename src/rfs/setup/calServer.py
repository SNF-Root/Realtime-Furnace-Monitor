...
def write_data(data, aruco_data):
    try:
        print(type(data))
        print(data)
        ...
        with open(ARUCO_COORD_FILE, "w") as f:
            json.dump([aruco_data], f, indent=2)
            print("aruco written succesfully")

    except Exception as e:
        print(e)

def add_data(new_data, new_aruco_data):
    try: 
        ...
        exist_aruco_data = aruco_read_data()
        exist_aruco_data.append(new_aruco_data)
        with open(ARUCO_COORD_FILE, "w") as f:
            json.dump(exist_aruco_data, f, indent=2)
    except Exception as e:
        print(e)
...
@app.post("/accepted")
def accepted(request: Request, p: Rect):
    global accepted_reqs
    xScale = (1920/1280)
    yScale = (1080/720)
    try:
        ...
        aruco_coords = ctx.get_id()
        for key, value in aruco_coords.items():
            if value[0] < (finalX + 100) and value[0] > (finalX - 100):
                aruco_data = {key: [value[0] - finalX, value[1] - finalY, finalW, finalH]}
        if accepted_reqs == 0:
            write_data(data, aruco_data)
        else:
            add_data(data, aruco_data)
        accepted_reqs+=1
        print(accepted_reqs)
        if accepted_reqs == 12:
            return JSONResponse({"limit": "achieved", "status": 200})
...
def run_calServer(ctx: calContext):
